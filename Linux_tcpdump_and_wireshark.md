...
- 优先级：not > and > or
- 使用括号 () 控制逻辑分组（必须加引号或转义）

```shell
# 抓取 (源是 A 或 B) 且 目标端口是 22
...
- 优先使用 not、and、or（可读性更好，无需担心 shell 转义）
- 复杂表达式用单引号包裹，避免 shell 干扰： 

```shell
tcpdump -i eth0 'host 10.0.0.5 and (port 80 or port 443)'
```

- 避免直接使用 !、&&、||，除非你明确处理了 shell 转义
